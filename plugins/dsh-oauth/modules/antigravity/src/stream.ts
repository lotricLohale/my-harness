  ...
  switch (message.stopReason) {
    case "stop":
      return message.content.length === 0
        ? {
            kind: "error",
            failure: {
              message: "Antigravity returned an empty response",
    ...
    case "aborted":
      return {
        kind: "aborted",
        failure: {
          message: message.errorMessage ?? "Antigravity request aborted",
          code: "ABORTED",
        },
      };
    case "error":
      return {
        kind: "error",
        failure: {
          message: message.errorMessage ?? "Antigravity request failed",
    ...
    case "deferred":
      return {
        kind: "error",
        failure: {
          message: `Antigravity ended with unsupported state ${message.stopReason}`,
          code: "ANTIGRAVITY_ERROR",
        },
      };
  }
...
export async function* toStreamChunks(
  ...
  throw new LlmError(
    "Antigravity stream ended without a terminal event",
    "STREAM_CLOSED",
  );
}
