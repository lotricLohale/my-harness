...
import {
	...
	type AntigravityAccountPayload,
} from "./web-data.js";
...
export function accountCandidates(
	accounts: readonly AntigravityAccountMeta[],
	now = Date.now(),
): AntigravityAccountMeta[] {
	return accounts
		.filter(
			(account) =>
...
export class AntigravityAuth {
	...
	displayName(): string {
		...
		if (accounts.length === 1)
			return `Antigravity（${accountLabel(accounts[0]!)}）`;
		const current =
	...
	async apiKeyFor(account: AntigravityAccountMeta): Promise<string> {
		...
		if (hit === undefined) {
			throw new LlmError(
				`Antigravity account ${accountLabel(account)} has no credential (${ref})`,
				"MISSING_CREDENTIAL",
			);
		}
	...
	async beginLogin(): Promise<LoginAttempt> {
		if (this.loginTask !== undefined) {
			throw new Error(
				"Antigravity login is already waiting for a browser callback",
			);
		}
	...
	async accountPayloads(
		includeQuota = true,
	): Promise<AntigravityAccountPayload[]> {
		await this.includeLegacyCredential();
		return Promise.all(
	...
	/** 删除指定账号的元数据和对应凭据。 */
	async removeAccount(id: string): Promise<AntigravityAccountMeta> {
		const account = this.currentAccounts().find((item) => item.id === id);
		if (account === undefined) {
			throw new Error(`Antigravity account "${id}" was not found`);
		}
	...
	private async includeLegacyCredential(): Promise<void> {
		...
		if (
			this.currentAccounts().some(
				(account) =>
					account.id === id || (email !== undefined && account.email === email),
			)
		)
			return;
		await this.storeAccounts([
...
export function parseCredentials(value: string): OAuthCredentials {
	...
	try {
		...
		throw new LlmError(
			"Antigravity credentials contain invalid JSON; run /antigravity-login",
			"INVALID_CREDENTIAL",
			{ cause: error },
		);
	}
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
		throw new LlmError(
			"Antigravity credentials must be a JSON object; run /antigravity-login",
			"INVALID_CREDENTIAL",
		);
	}
	...
	if (
		...
		throw new LlmError(
			"Antigravity credentials are incomplete; run /antigravity-login",
			"INVALID_CREDENTIAL",
		);
	}
