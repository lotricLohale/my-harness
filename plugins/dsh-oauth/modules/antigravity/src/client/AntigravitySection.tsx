...
import {
	...
	type AntigravityAccountsResponse,
} from "../web-data.js";
...
async function loadAccounts(
	includeQuota = true,
): Promise<AntigravityAccountsResponse> {
	const path = includeQuota
		? "/api/antigravity/accounts"
		: "/api/antigravity/accounts?usage=0";
	const res = await fetch(path, { headers: { Accept: "application/json" } });
	const body = (await res.json()) as Partial<AntigravityAccountsResponse> & {
		error?: string;
	};
...
async function deleteAccount(id: string): Promise<void> {
	const res = await fetch(
		`/api/antigravity/accounts/${encodeURIComponent(id)}`,
		{
			method: "DELETE",
...
export function AccountCard({
	...
	account: AntigravityAccountPayload;
					...
					<div style={{ display: "grid", gap: 6, marginTop: 6 }}>
						{group.buckets.map((bucket) => (
							<div key={`${group.displayName}:${bucket.displayName}`}>
								<span>
									{bucket.displayName}：
									{formatPercent(bucket.remainingFraction)} 剩余
								</span>
...
export function AntigravityAccountsPanel() {
	...
	const refresh = async (
		includeQuota = true,
	): Promise<AntigravityAccountsResponse | undefined> => {
		try {
			if (includeQuota)
	...
	return (
		<section>
			<h3 style={{ marginTop: 0 }}>Antigravity (OAuth)</h3>
			<p>
				账号、五小时/每周额度和授权状态。Token 只保存在 Host
				credentials，不会发送到浏览器。
			</p>
