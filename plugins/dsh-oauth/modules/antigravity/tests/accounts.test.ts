...
function provider(
	streamSimple: (...args: any[]) => AsyncIterable<AssistantMessageEvent>,
): any {
	return {
		name: "Antigravity",
		api: "antigravity-api",
...
function auth(accounts: AntigravityAccountMeta[]): any {
	...
	return {
		...
		markExhausted: async (account: AntigravityAccountMeta) => {
...
test("没有授权账号时模型列表为空", async () => {
	...
	assert.deepEqual(await adapter.listModels("antigravity"), []);
});

test("有授权账号时才列出模型分组", async () => {
	const adapter = new AntigravityAdapter(
		provider(() => okEvents()),
		auth([{ id: "a", credentialRef: "A" }]),
	);
...
test("删除账号会移出账号池并清除凭据", async () => {
	const unset: string[] = [];
	const stored: { accounts?: AntigravityAccountMeta[] } = {
		accounts: [
			{ id: "keep", credentialRef: "ANTIGRAVITY_OAUTH_KEEP" },
			{ id: "gone", credentialRef: "ANTIGRAVITY_OAUTH_GONE" },
		],
	};
	const { AntigravityAuth } = await import("../src/auth.js");
	...
	assert.deepEqual(unset, ["ANTIGRAVITY_OAUTH_GONE"]);
});
