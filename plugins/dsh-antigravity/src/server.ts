...
/** 写 JSON 响应。 */
export function writeJson(res: Pick<ServerResponse, 'writeHead' | 'end'>, status: number, body: unknown): void {
...
export function assertSameOriginJson(req: Pick<IncomingMessage, 'headers'>): string | undefined {
  ...
  if (typeof origin === 'string') {
    ...
    if (typeof host !== 'string' || new URL(origin).host !== host) return 'cross-origin requests are not allowed'
  }
  return undefined
}
...
export function registerWebRoutes(ctx: Context, auth: AntigravityAuth): void {
  ctx.inject(['webServer'], (wctx) => {
    const webServer = (wctx as unknown as { webServer: WebServerLike }).webServer
    const accountRoute: WebRoute = {
      ...
      handler: async (req, res) => {
        ...
        const includeQuota = new URL(req.url ?? ACCOUNTS_PATH, 'http://localhost').searchParams.get('usage') !== '0'
        writeJson(res, 200, {
          accounts: await auth.accountPayloads(includeQuota) satisfies AntigravityAccountPayload[],
