...
/** 将 pi-antigravity 的终止原因转换成 Harness 协议。 */
export function mapStopReason(message: AssistantMessage): FinishReason {
  switch (message.stopReason) {
    case 'stop': return message.content.length === 0
      ? { kind: 'error', failure: { message: 'Antigravity returned an empty response', code: EMPTY_RESPONSE_CODE } }
      : { kind: 'stop' }
    case 'length': return { kind: 'max-tokens' }
    case 'toolUse': return { kind: 'tool-calls' }
    case 'aborted': return {
      kind: 'aborted',
      failure: { message: message.errorMessage ?? 'Antigravity request aborted', code: 'ABORTED' },
    }
    case 'error': return {
      kind: 'error',
      failure: { message: message.errorMessage ?? 'Antigravity request failed', code: 'ANTIGRAVITY_ERROR' },
    }
    case 'pending':
    case 'deferred': return {
      kind: 'error',
      failure: { message: `Antigravity ended with unsupported state ${message.stopReason}`, code: 'ANTIGRAVITY_ERROR' },
    }
  }
}

/** 将 pi-antigravity 事件流转换成完整的 Harness StreamChunk 序列。 */
export async function* toStreamChunks(events: AsyncIterable<AssistantMessageEvent>): AsyncGenerator<StreamChunk> {
  ...
  throw new LlmError('Antigravity stream ended without a terminal event', 'STREAM_CLOSED')
}
