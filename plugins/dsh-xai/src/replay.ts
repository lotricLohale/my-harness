...
export function toReplayState(message: AssistantMessage): XaiReplayState {
	return {
		kind: "xai-pi-ai",
		version: 1,
		api: message.api,
...
function foreignAssistant(message: Message): AssistantMessage {
	...
	for (const block of message.content) {
		switch (block.type) {
			...
			case "image":
				throw new LlmError(
					"xAI assistant image history is unsupported",
					"UNSUPPORTED_CONTENT",
				);
			default:
				break;
...
function replayedAssistant(
	...
	const state = raw as Partial<XaiReplayState>;
	if (
		state.kind !== "xai-pi-ai" ||
		state.version !== 1 ||
		state.provider !== source.provider ||
		state.model !== source.model ||
		!Array.isArray(state.blocks) ||
		state.blocks.length !== message.content.length
	) {
		return foreignAssistant(message);
	}
	const content: AssistantMessage["content"] = message.content.map(
		(block, index) => {
			...
			switch (block.type) {
				...
				default:
					throw new LlmError(
						`xAI cannot replay assistant block "${block.type}"`,
						"UNSUPPORTED_CONTENT",
					);
			}
		},
